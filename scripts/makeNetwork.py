...
def mountImage(targetDir):
    ...
    # 使用 noauto,nosuid,nodev,noexec,x-gvfs-hide 避免桌面自动挂载/打开，并禁止执行
    os.system('mount -o noauto,nosuid,nodev,noexec,x-gvfs-hide %s %s/image > /dev/null' % (loopFile, targetDir))
    time.sleep(1)
    return loopFile
